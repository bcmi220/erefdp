...
def read_conll(fh, lang='en', proj_cons=False):
	read_in_sent = 0
	drop_sent = 0
	root = ConllEntry(0, '<ROOT>', 'ROOT_POS', 'ROOT_POS', 'ROOT_POS', 0, 'rroot')
	...
	for line in fh:
		...
		if not tok:
			...
			tokens.append(ConllEntry(int(tok[0]), tok[1], tok[3], 
							...
							if tok[6] is not '_' else -1, tok[7]))
	...
	with open(filename, 'w') as fh:
		for sentence in conll_gen:
			for entry in sentence[1:]:
				...
				fh.write('\t'.join([str(entry.w_id), entry.form, '_', \
					entry.gpos, entry.ppos, '_', str(entry.pred_pid), \
					entry.pred_rel, str(entry.order), str(entry.margin)]))
				# debug end
...
_numberRegex = re.compile('[0-9]+|[0-9]+\\.[0-9]+|[0-9,]+')
def _normalize(word):
